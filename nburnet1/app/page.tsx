  ...
  return (
    <>
      ...
      <section className="flex flex-col gap-4 pb-8">
        <h3 className={title({ size: "sm", class: "mt-4 text-foreground-500" })}>
          Work Experience
        </h3>
        {Resume.work.map((work, index) => (
          <div key={index} className="flex flex-col opacity-100 gap-2 lg:opacity-60 hover:opacity-100">
            <h4 className={subtitle({ class: "text-foreground" })}>
              {work.position}
            </h4>
            <p className="text-primary-700 flex md:hidden">{work.startDate} - {work.endDate == "" ? "Present" : work.endDate}</p>
            <div className="flex justify-between">
              <Link href={work.url} className="gap-2 text-primary-700"><p>{work.name}</p><LaunchIcon fontSize="small" className="text-default-500" /></Link>
              <p className="text-primary-700 hidden md:flex justify-end">{work.startDate} - {work.endDate == "" ? "Present" : work.endDate}</p>
            </div>

            <ul className="list-disc list-inside">{work.highlights && work.highlights.map((highlight, index) => (
      ...
      <section className="flex flex-col gap-4 pb-8">
        ...
        {Resume.education.map((education, index) => (
          <div key={index} className="flex flex-col opacity-100 gap-2 lg:opacity-60 hover:opacity-100">
            <h4 className={subtitle({ class: "text-foreground" })}>
              {education.studyType ? `${education.studyType} in ${education.area}` : education.area}
            </h4>
            <p className="text-primary-700 md:hidden flex ">{education.startDate} - {education.endDate}</p>
            <div className="flex justify-between">
              <p className="text-primary-700">{education.institution}</p>
              <p className="text-primary-700 md:flex justify-end hidden ">{education.startDate} - {education.endDate}</p>
            </div>
      ...
      <section className="flex flex-col gap-4 pb-8">
        ...
        {Resume.projects.map((project, index) => (
          <div key={index} className="flex flex-col gap-2 opacity-100 lg:opacity-60 hover:opacity-100">
            <Link href={project.url} className="flex ">
            <h4 className={subtitle({fullWidth: false, class: "w-min text-foreground item-center" })}>
                {project.name} 
            </h4>
              <LaunchIcon fontSize="small" className="text-default-500" />
            
            </Link>
            <p className="text-primary-700 flex md:hidden">{project.startDate} - {project.endDate == "" ? "Present" : project.endDate}</p>
            <div className="flex justify-between">
              <p className="text-primary-700">{project.description}</p>
              <p className="text-primary-700 justify-end hidden md:flex">{project.startDate} - {project.endDate == "" ? "Present" : project.endDate}</p>
            </div>

            <div>
              <ul className="list-disc list-inside">{project.highlights && project.highlights.map((highlight, index) => (
                <li key={index}>{highlight}</li>
              ))}</ul>
            </div>
          </div>
